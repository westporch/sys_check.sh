...
function REFINE_SYSLOG()
{
    ...
    cat $ENTIRE_SYSLOG | grep -Ev "Connection from UDP|orchard|irqbalance|drop_caches|MTA|CRON|rsyslogd|normally" > $REFINED_SYSLOG
...
function REFINE_KERN_LOG()
{
    ...
    cat $ENTIRE_KERN_LOG | grep -Ev "promiscuous|vif|IPv6" > /tmp/refined_kern.log
